...
\caption{Performance comparison of detection rates across three models: (a) PoA-Only (85\%), (b) Reactive-Only (87\%), (c) Our Combined Model (98\%). The two-layer approach significantly outperforms single-layer baselines.}\label{fig:detection_comparison}
\end{figure}
...
\caption{Trust calculation transparency comparison: (a) Black-box system with hidden formulas, (b) Our transparent 5-metric system showing all calculations explicitly. Every metric's contribution is visible and verifiable.}\label{fig:transparency}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{graph2_election_mechanism.png}
\caption{Complete election mechanism flowchart showing the transparent 5-metric calculation process with explicit weights (40-20-15-15-10) and security layer integration (PoA + Sleeper Detection).}\label{fig:election_mechanism}
\end{figure}
...
\caption{Comprehensive comparison of the five metrics (Trust, Resource, Stability, Behavior, Centrality) across different node types: benign high-performers, average nodes, and detected malicious nodes. The metric profiles clearly differentiate trustworthy from untrustworthy nodes.}\label{fig:5metric_comparison}
\end{figure}
